...
mcp_instructions="$(
    get_config_value \
        "MCP_INSTRUCTIONS" \
        "This is a ${program_name} instance managing git repositories on host $(hostname) ($(hostname -I 2>/dev/null | awk '{print $1}' || echo 'unknown')). Before running destructive or mutating operations, verify that this is the correct server for your intended target."
)"
